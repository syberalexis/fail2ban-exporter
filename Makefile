empty :=
PROJECT_NAME := fail2ban-exporter
DIST_FOLDER := dist
...
docker:
	docker build -t syberalexis/fail2ban-exporter:latest --build-arg VERSION=$(VERSION) .
ifneq ($(VERSION),)
	docker tag syberalexis/fail2ban-exporter syberalexis/fail2ban-exporter:$(VERSION)
endif
